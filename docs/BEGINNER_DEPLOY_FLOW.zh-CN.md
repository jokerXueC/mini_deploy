...
面板的“自动初始化”和“服务器执行指令”里都会加入 `git ls-remote` 检查。如果权限不通，会直接提示你去配置 SSH Key，不会继续往下部署。

## 6. 自动初始化和“服务器执行指令”是什么

添加项目时，优先点击“自动初始化”。它会让 Agent 直接在服务器上准备项目目录、拉代码、写入 `deploy.sh`，并为 Python / Go / Java 生成一份初版 systemd service。
