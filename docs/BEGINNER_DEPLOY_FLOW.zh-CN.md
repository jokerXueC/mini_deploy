...
flowchart TD
    ...
    G --> I[添加项目]
    I --> I1[识别仓库并确认类型/启动命令/端口]
    I1 --> I2[预览部署文件并勾选确认]
    I2 --> J[点击自动初始化]
    J --> K[拉取代码并补齐脚本/service，保留已有文件]
    K --> L[点击检查项目]
    L --> M{检查通过?}
...
“识别仓库”会验证指定分支能否读取，“自动初始化”也会检查仓库访问。失败时按提示区分 SSH 权限、HTTPS 凭据、分支和网络问题，不会继续初始化。

## 6. 自动初始化和“服务器执行指令”是什么

添加项目时，先识别仓库并确认配置，再点击“预览部署文件”，勾选确认后点击“自动初始化”。它会准备目录、拉代码，补齐缺少的 `deploy.sh` 和 Python / Go / Java 初版 systemd service；已有文件会保留。初始化不会启动业务，运行环境齐备后再启用项目并手动部署测试。
...
面板可以帮你生成基础脚本和 systemd 初版 service，但它不知道你的真实项目细节。
